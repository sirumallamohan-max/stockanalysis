...
time_step = st.sidebar.slider("Time Steps (Lookback period)", min_value=10, max_value=100, value=30, step=5)
...
if ticker:
    ...
    if df.empty:
        ...
        if st.button(f"Predict Next {days} days"):
            ...
            future_dates = pd.date_range(start=pd.Timestamp.today().normalize() + pd.Timedelta(days=1), periods=days, freq='D')
            data = pd.DataFrame({'Date':future_dates,'Close':predicted_prices})
            data.set_index('Date', inplace=True)
            st.subheader(f"Next {days} Days Predicted Closing Prices")
            st.dataframe(data)
            st.line_chart(data)
            print(f"\n--- Next {days} Days Predicted Closing Prices ---")
            for i, price in enumerate(predicted_prices, 1):
                #print(f"Day {i}: {price:.2f}")
                st.write(f"Day {i}: {price:.2f}")
            print("\n--- Predicted of Closing Prices Done ---")